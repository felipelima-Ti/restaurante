...
const OrderList = ({ orders }: OrderListProps) => {
  ...
  // Aguarda a navegação voltar antes de forçar o reload
  setTimeout(() => {
    if (typeof window !== "undefined") {
      location.reload();
    }
  }, 100); // meio segundo de atraso (ajustável)
};
    ...
    {orders.map((order) => (
        <Card key={order.id}>
            <CardContent className="space-y-4 p-5 border border-gray-200">
                ...
                ${order.status == OrderStatus.FINISHED ? "bg-green-500 text-white" : "bg-gray-400 text-gray-500"}
                    ${order.status == OrderStatus.IN_PREPERATION ? "bg-orange-400 text-white" : "bg-gray-400 text-gray-500"}
                        ${order.status == OrderStatus.DELIVERY ? "bg-green-400 text-white" : "bg-gray-400 text-gray-500"}
                        `}
                        >
            ...
            {getStatusLabel(order.status)}
                ...
                <div className="flex items-center gap-2">
                    <div className="ralative h-4 w-5">
                        <Image
